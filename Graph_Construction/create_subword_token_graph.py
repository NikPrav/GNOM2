...
"""
__synopsis__    : Handling subword tokens.
__description__ :
...
def get_subword_vocab(data_filename, data_dir=dataset_dir, tokenizer=BERT_tokenizer(),
    ...
    if exists(fullpath + '_corpus_toks.json')\
        ...
        if get_clean_corpus:
            ...
            # else:
            #     low_oov_ignored.update(set(vocab['freqs'].keys()) - set(vocab['idx2str_map']))
...
def create_token_graph(joint_path, all_source_path, target_path,
    ...
    low_oov_ignored = set(joint_vocab['freqs'].keys()) - set(joint_vocab['idx2str_map'])
...
def get_node_vectors(G, s2i, X):
    ...
    for node_id, data in G.nodes(data=True):
        if data['node_txt'] == '<unk>' or data['node_txt'] == '<pad>':
            continue
        idx = s2i.get(data['node_txt'], 0)
        node_txt2embs[data['node_txt']] = X[idx]
