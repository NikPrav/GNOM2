...
def get_new_pretrain_dataset(G, G_pos, G_neg, joint_vocab, pos_vocab, neg_vocab,
    ...
    # assert pos_neg_union == set(joint_vocab['str2idx_map'].keys()),\
    #     f'{len(pos_neg_union)} == {len(joint_vocab["str2idx_map"].keys())} did not match.'
    logger.info(f"Overlap between selected and total vocab: {(len(Cpos.union(Cneg)) / len(pos_neg_union)) * 100}:2.4")
...
def get_subword_vocab_data(path=join(pretrain_dir, data_filename + "_multihot.csv"),
    ...
    if exists(join(pretrain_dir, data_filename + name + '_corpus_toks.json'))\
        ...
        corpus_strs, corpus_toks, ignored_examples =\
            create_clean_corpus(dataset, low_oov_ignored=list(set(vocab['freqs'].keys()) - set(vocab['idx2str_map'])))
...
def get_graph_and_dataset(limit_dataset=None):
    joint_vocab, joint_corpus_toks, joint_corpus_strs =\
        get_subword_vocab_data(name='_subword_joint', read_input=True)
    ...
    pos_vocab, pos_corpus_toks, pos_corpus_strs =\
        get_subword_vocab_data(pos_path, name='_subword_pos')
    ...
    neg_vocab, neg_corpus_toks, neg_corpus_strs =\
        get_subword_vocab_data(neg_path, name='_subword_neg')
