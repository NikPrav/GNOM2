...
class ClassificationModel:
    ...
    def train_model(self, train_df, multi_label=False, output_dir=None,
    ...
    def train(self, train_dataloader, output_dir, multi_label=False,
    ...
    def eval_model(self, eval_dicts=None, eval_df=None, multi_label=False,
    ...
    def evaluate(self, eval_df, output_dir, multi_label=False,
...
def main(args, multi_label=False):
    model_args = set_model_args(n_classes=1, lr=args.lr,
    ...
    clf = ClassificationModel(
        ...
        num_labels=1, args=model_args, exp_name=args.exp_name,
        use_cuda=args.use_cuda, cuda_device=cuda_device)

    init_embs = clf.bert_model.bert.embeddings.word_embeddings.weight.data

    knn = neighbors.NearestNeighbors(n_neighbors=5, radius=0.2)
    knn.fit(init_embs)
    k = 5
    token_id = 3057
    dists, idxs = knn.kneighbors(init_embs[token_id].unsqueeze(0), k)
    logger.info(clf.tokenizer.decode(token_id))
    logger.info(clf.tokenizer.decode(idxs[0]))
    logger.info(dists)
