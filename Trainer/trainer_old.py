...
"""
__synopsis__    : Builds the experiment using matching network, the training and evaluation ops as well as
dataloader augmentation routines.

__description__ : Builds the experiment using matching network, the training and evaluation ops as well as
dataloader augmentation routines.
__project__     : MNXC
...
class Run_Network:
    ...
    def __init__(self, Model, data_formatter, use_cuda: bool = config["model"]["use_cuda"],
                 batch_size: int = config['training']["train_batch_size"],
                 lr_decay: int = config["model"]["optimizer"]["lr_decay"],
                 lr: float = config["model"]["optimizer"]["lr"]) -> None:
        ...
        :param data_formatter: A dataloader provider class
        ...
        layer_size = config['training']["categories_per_batch"] * config['training']["supports_per_category"]
        self.model = Model(layer_size=layer_size, num_channels=layer_size)
    ...
    def training(self, num_train_epoch: int, view_grads: bool = config["model"]["view_grads"],
        ...
        optimizer = self.__create_optimizer(self.model, self.lr)  ## Creating the optimizer

        with tqdm.tqdm(total=num_train_epoch) as pbar:
            self.model.train()
            for i in range(num_train_epoch):  ## 1 train epoch
                logger.info("Total epoch: [{}]".format(self.total_train_iter))
                ...
                ## Print Model Summary:
                # make_dot(loss, self.model)
                # hl.build_graph(self.model, args=(x_supports, y_support_hots, x_hats, y_hats_hots,
                # target_cat_indices))

                if self.cuda_available and self.use_cuda:
                    loss, targets_preds = self.model(x_supports.cuda(), y_support_hots.cuda(), x_hats.cuda(),
                                                     y_hats_hots.cuda(), target_cat_indices,
                                                     batch_size=self.batch_size)
                else:
                    loss, targets_preds = self.model(x_supports, y_support_hots, x_hats, y_hats_hots,
                ...
                if view_grads and self.total_train_iter % 5 == 0:
                    logger.debug(self.model.named_modules())
                    for name, param in self.model.named_parameters():
                        logger.debug((name, param.data.shape, param.grad.shape))
                    # logger.debug(self.model.state_dict())
                ...
                if view_train_precision:
                    ...
                    precision_1 = precision_k_hot(y_hats_hots, targets_preds, k=1)
                    logger.info("Precision @ 1: {}".format(precision_1))
                    precision_3 = precision_k_hot(y_hats_hots, targets_preds, k=3)
                    logger.info("Precision @ 3: {}".format(precision_3))
                    precision_5 = precision_k_hot(y_hats_hots, targets_preds, k=5)
    ...
    def validating(self, epoch_count: int, num_val_epoch: int = 1) -> float:
        ...
        with tqdm.tqdm(total=num_val_epoch) as pbar:
            ...
            for i in range(num_val_epoch):  ## 1 validation epoch
                ...
                if self.cuda_available and self.use_cuda:
                    loss, targets_preds, encoded_x_hat = self.model(x_supports.cuda(), y_support_hots.cuda(),
                                                                    x_targets.cuda(), y_target_hots.cuda(),
                                                                    batch_size=self.batch_size,
                                                                    requires_grad=False, print_accuracy=True)
                else:
                    loss, targets_preds = self.model(x_supports, y_support_hots, x_targets,
                ...
                precision_1 = precision_k_hot(y_target_hots, targets_preds, k=1)
                logger.info("Precision @ 1: {}".format(precision_1))
                precision_3 = precision_k_hot(y_target_hots, targets_preds, k=3)
                logger.info("Precision @ 3: {}".format(precision_3))
                precision_5 = precision_k_hot(y_target_hots, targets_preds, k=5)
                logger.info("Precision @ 5: {}".format(precision_5))
        ...
        with tqdm.tqdm(total=total_test_batches) as pbar:
            with torch.no_grad():
                for i in range(total_test_batches):  ## 1 test epoch
                    ...
                    if self.cuda_available and self.use_cuda:
                        loss, targets_preds = self.model(x_supports.cuda(), y_support_hots.cuda(),
                                                         x_targets.cuda(), y_target_hots.cuda(),
                                                         target_cat_indices)
                    else:
                        loss, targets_preds = self.model(x_supports, y_support_hots, x_targets, y_target_hots,
                                                         target_cat_indices)
    ...
    def __create_optimizer(self, model, new_lr, optimizer_type=config["model"]["optimizer"]["optimizer_type"],
        ...
        if optimizer_type == 'sgd':
            optimizer = torch.optim.SGD(
                model.parameters(), lr=new_lr, momentum=momentum,
                dampening=dampening, weight_decay=weight_decay)
        elif optimizer_type == 'adam':
            optimizer = torch.optim.Adam(model.parameters(), lr=new_lr,
                                         weight_decay=weight_decay)
        elif optimizer_type == 'adadelta':
            optimizer = torch.optim.Adadelta(model.parameters(), lr=new_lr,
                                             rho=rho, weight_decay=weight_decay)
        elif optimizer_type == 'adagrad':
            optimizer = torch.optim.Adagrad(model.parameters(), lr=new_lr,
                                            lr_decay=self.lr_decay,
                                            weight_decay=weight_decay)
        elif optimizer_type == 'rmsprop':
            optimizer = torch.optim.RMSprop(
                model.parameters(), lr=new_lr, alpha=alpha, momentum=0.9,
...
if __name__ == '__main__':
    ...
    cls = Run_Network(input_size=cfg['embeddings']['emb_dim'], hid_size=100,
                      fce=True, batch_size=32, supports_per_category=15,
