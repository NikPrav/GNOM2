...
from config import configuration as cfg, platform as plat, username as user,\
    pretrain_dir, cuda_device, emb_dir
...
def train_lstm_classifier(
        ...
        val_dataloader: utils.data.dataloader.DataLoader = None,
        model_name='declutr'):
    ...
    for epoch in range(1, epoch + 1):
        ...
        for iter, (emb, label) in enumerate(dataloader):
            ...
            prediction = model(emb.unsqueeze(1))
            if cfg['cuda']['use_cuda'][plat][user] and cuda.is_available():
        ...
        if val_dataloader is not None:
            val_losses, val_output = eval_lstm_classifier(
                model, loss_func=loss_func, dataloader=val_dataloader)
            logger.info(
                f"Epoch {epoch}, time: {train_time / 60:1.4} mins, Train loss: "
                f"{epoch_loss:0.6} Val W-F1 {val_output['result']['f1_weighted'].item():4.4}"
                f" TEST W-F1: [{test_output['result']['f1_weighted']:1.4}] Model {model_name}"
                f" Dataset {cfg['data']['test']}\n{dumps(test_output, indent=4)}")
        else:
            logger.info(
                f"Epoch {epoch}, time: {train_time / 60:1.4} mins, Train loss: "
                f"{epoch_loss:1.6} TEST W-F1: [{test_output['result']['f1_weighted']:1.4}]"
    ...
    logger.info(
        f"DECLUTR Epoch {max_result['epoch']}, MAX Model {model_name} MAX Score "
...
def eval_all(model: BiLSTM_Classifier, loss_func,
    ...
    if all_test_dataloaders is None:
        ...
        for tfile in test_files:
            ...
            dataset, dataloader = get_declutr_dataloader(test_df)
...
def declutr_trainer(
    ...
    if pretrain_dataloader is not None:
        ...
        model, epoch_losses, train_epochs_output_dict = train_lstm_classifier(
            ...
            epoch=pretrain_epoch, test_dataloader=test_dataloader,
    ...
    model, epoch_losses, train_epochs_output_dict = train_lstm_classifier(
        ...
        epoch=epoch, val_dataloader=val_dataloader, test_dataloader=test_dataloader, model_name=model_name)
...
def main():
    ...
    :return:
    """
    declutr_trainer(in_dim=1, hid_dim=4, num_heads=2)
