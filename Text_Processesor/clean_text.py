...
wiki_patterns = ("It has been suggested that Incremental reading be merged into this article or section. (Discuss)",
                 ...
                 'because it lacks inline citations. Please improve this article by introducing more precise '
...
class Clean_Text(object):
    ...
    def __init__(self, dataset_name: str = config["data"]["dataset_name"],
                 data_dir: str = config["paths"]['dataset_root'][plat][user]):
    ...
    def calculate_idf(self, docs: list, subtract: int = 1) -> dict:
        ...
        idf = vectorizer.idf_
        idf_dict = dict(
            zip(vectorizer.get_feature_names(), idf - subtract))  ## Subtract 1 from idf to get better scores
    ...
    def clean_doc(self, doc: list, pattern=re.compile(r"\[\d\]"), replace='',
                  symbols=('_', '-', '=', '@', '<', '>', '*', '{',
                           '}', '[', ']', '(', ')', '$', '%', '^', '~',
                           '`', ':', "\"", "\'", '\\', '/', '|', '#',
                           '##', '###', '####', '#####')) -> list:
...
Look up encryption in Wiktionary, the free dictionary.
  ...
  * Accumulative archive of various cryptography mailing lists. Includes Cryptography list at metzdowd and 
